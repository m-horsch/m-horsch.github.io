...
This strategy clones the most promising hikers, leaving the hikers at unpromising locations untouched for the time being.  If any of the new clones happen to be placed on an unpromising location, we let them stay there while we clone other hikers.  The notion of being in a promising location is relative: we compare the locations of all the hikers, and clone the most promising one.  The most promising one can change at every turn.  Hikers in locations that seemed unpromising may eventually be the best, which could happen if all the other hikers get stuck in even worse locations.
